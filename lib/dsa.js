...
var wwPath = 'dsa-webworker.js'
var Worker = global.Worker
if (!Worker) {
  wwPath = path.join(__dirname, wwPath)
  try {
...
DSA.verify = function (key, m, r, s) {
  ...
  var hm = msgToBigInt(normalizeMsg(m))
...
function normalizeMsg (m) {
  if (typeof Buffer !== 'undefined' && Buffer.isBuffer(m)) {
    return CryptoJS.enc.Hex.parse(m.toString('hex'))
  }
  else {
    return CryptoJS.enc.Latin1.parse(m)
  }
}

function msgToBigInt (m) {
  return BigInt.str2bigInt(m.toString(CryptoJS.enc.Hex), 16)
}
